...
module Seahorse
  module Client
    module Http
      Spectator.describe Headers do
        include Seahorse::Client::Http
        ...
        describe "#each" do
          ...
          it "is enumerable" do
            expect(headers).to be_kind_of(Enumerable(Tuple(String, String)))
          end
...
#            #expect(headers.each).to be_kind_of(Enumerator)
        ...
        describe "#keys" do
          ...
          it "returns the keys" do
            headers["key1"] = "value1"
            headers["key2"] = "value2"
            expect(headers.keys).to eq(["key1", "key2"])
          end
        ...
        describe "#values" do
          ...
          it "returns the values" do
            headers["key1"] = "value1"
            headers["key2"] = "value2"
            expect(headers.values).to eq(["value1", "value2"])
          end
        ...
        describe "#values_at" do
          ...
          it "returns the values for the given keys" do
            headers["key1"] = "v1"
            headers["key2"] = "v2"
            expect(headers.values_at("key2", "key1")).to eq({"v2", "v1"})
          end
        ...
        describe "#has_key?" do
          ...
          it "returns false if the header has not been set" do
            expect(headers.has_key?("foo")).to be(false)
          end
